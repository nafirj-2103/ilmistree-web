...
export function SecondaryNavbar() {
  ...
  return (
    <nav className={`w-full bg-gray-700 border-b border-gray-600 fixed top-[73px] z-40 transition-transform duration-300 backdrop-blur-sm ${isVisible ? 'translate-y-0 opacity-100 pointer-events-auto' : '-translate-y-full opacity-0 pointer-events-none'}`} suppressHydrationWarning>
      <div className="max-w-7xl mx-auto px-20 h-[20px] flex items-center gap-8">
        {navSections.map((section) => (
          <div key={section.label} className="relative group">
            <span
              className="flex items-center gap-2 text-white hover:text-[#D32F2F] transition-colors mt-[-0px] cursor-default select-none"
            >
              {section.label}
            ...
            <ul className="absolute top-full left-0 hidden group-hover:block bg-gray-800 border border-gray-600 rounded-sm shadow-lg min-w-[200px] py-2 z-50 mt-[-2px]">
              {section.items.map((item) => (
                <li key={item.label}>
                  <span
                    className="w-full text-left px-4 py-2 text-white hover:bg-[#D32F2F] hover:text-white transition-colors font-medium block"
                  >
                    {item.label}
                  </span>
                </li>
