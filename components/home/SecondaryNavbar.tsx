      ...
      <div className="max-w-7xl mx-auto px-20 h-[20px] flex items-center gap-8">
        {navSections.map((section) => (
          <div key={section.label} className="relative group">
            ...
            <button
              className="flex items-center gap-2 text-white hover:text-[#D32F2F] transition-colors mt-[-0px] cursor-default pointer-events-none"
            >
              {section.label}
              <ChevronDown className="w-4 h-4" />
            </button>

            <ul className="absolute top-full left-0 hidden group-hover:block bg-gray-800 border border-gray-600 rounded-sm shadow-lg min-w-[200px] py-2 z-50 mt-[-2px] pointer-events-none">
              {section.items.map((item) => (
                <li key={item.label}>
                  <span
                    className="w-full text-left px-4 py-2 text-white hover:bg-[#D32F2F] hover:text-white transition-colors font-medium cursor-default"
                  >
                    {item.label}
                  </span>
                </li>
